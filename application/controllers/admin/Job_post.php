...
class Job_post extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 public function index()
     {
		...
		$data['minimum_education'] = $this->Xin_model->getMinEducation();
	...
	public function add_job() {
		...
		$min_edu = $this->input->post('min_education');

		$result = $this->Job_post_model->add($data, $min_edu);
		if ($result == TRUE) {
	...
	public function PushNotifications($fields)
	{
	   $api_key = "AAAARRBc8jA:APA91bGrHMmZw_8APytswa_dhkAhtmCbBuxSPS7noQPwGZDL6nist2CFvAWlvKRKGlA2qdE-koNwS4r-OK4h9-Sr2uXolJK40KV-vYwBYpSCNIqV-UEiHqoJE1YTuvj-1zd2KjQwZ35h";
	   $headers = array
		   (
			   'Authorization: key='.$api_key,
			   'Content-Type: application/json'
		   );
		$registrationId = $this->Notification_Model->getIdsEvents();
		foreach($registrationId as $r)
		{
		   $arrayToSend = array('to'=>$r['device_token'] , 'notification'=> $fields);
