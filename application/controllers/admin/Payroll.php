...
class Payroll extends MY_Controller {
	...
	public function output($Return=array()){
	 ...
	 //attendance 
	 public function attendance_total($id)
	 {
		$day = '01';
		$month = (date('m'))-1;
		$year = date('Y');
		...
		$imonth = $month; //date('F', $date);
		$r = $this->Xin_model->read_user_info($id);
		$pcount = 0;
		$h_count = 0;
		$acount = 0;
		$lcount = 0;
		...
		for($i = 1; $i <= $daysInMonth; $i++):
			...
			//	$acount += 0;
				...
				$status = $check->result_array();
				$status = $status[0]['attendance_status'];
				if($status == "Late")
				{
					$late += 1;
				}
		...
		$total_leaves = $this->Payroll_model->get_leaves_count($id, $month);
		$total_present_days = ($pcount+$late+$h_count+$total_leaves);
	...
	public function payroll_template_read()
	{
		...
		$data = array(
				...
				'absents' =>$this->attendance_total($user[0]->user_id)
				);
		if(!empty($session)){ 
	...
	public function pay_salary()
	{
		...
		$data = array(
				...
				'basic_salary' => $user[0]->basic_salary,
				'daily_wages' => $user[0]->daily_wages,
				'absent_count' => $this->attendance_total($user[0]->user_id)
				);
		if(!empty($session)){ 
...
if($month ==0)
{
	...
	// payment history
	 ...
	 public function pdf_create(){
		...
		// allowances
		$allowances_titles = array();
		$allowances_amount_in = array();
		$count_allowances = $this->Employees_model->count_employee_allowances_payslip($payment[0]->payslip_id);
		$allowances = $this->Employees_model->set_employee_allowances_payslip($payment[0]->employee_id);
		$total_allowancess = $this->Payroll_model->getAllowances($payment[0]->employee_id);
		if($total_allowancess != 0)
		{
			foreach($total_allowancess as $t)
			{
				$allowances_titles[] = $t['allowance_title'];
				$allowances_amount_in[] = $t['allowance_amount'];
			}
		}
		// commissions
		...
		// basic Salary
		$template = $this->Employees_model->getTemplate();
		$basic = $template[0]['basic_salary'];
		$medical = $template[0]['medical_allowance'];
		$transport = $template[0]['travelling_allowance'];
		$utilities = $template[0]['dearness_allowance'];
		$house_rent = $template[0]['house_rent_allowance'];
		$basic_salary = (($bs/100)*$basic);
		$medical_salary = (($bs/100)*$medical);
		$transport_salary = (($bs/100)*$transport);
		$utilities_salary = (($bs/100)*$utilities);
		$house_salary = (($bs/100)*$house_rent);
		// allowances
		...
		<table cellpadding="1" cellspacing="1" border="0">
			...
			<tr>
				<td align="center">'."Month Of".': '.$half_title.' <strong>'.date("F Y", strtotime($payment[0]->salary_month)).'</strong></td>
			</tr>
		</table>
		';
		...
		//// break..
		$pdf->Ln(7);
		$tblbrk = '<table cellpadding="3" cellspacing="0" border="1"><tr bgcolor="#A9A9A9">
			...
			if($payment[0]->payslip_type!='hourly'){
				$tblbrk .= '<tr><td colspan = "2">Basic Salary('.$basic.'%)</td><td align="right"  valign="bottom">'.$this->Xin_model->currency_sign($basic_salary).'</td></tr>
				<tr><td colspan = "2">House Rent Allowance('.$house_rent.'%)</td><td align="right"  valign="bottom">'.$this->Xin_model->currency_sign($house_salary).'</td></tr>
				<tr><td colspan = "2">Utilities Allowance('.$utilities.'%)</td><td align="right"  valign="bottom">'.$this->Xin_model->currency_sign($utilities_salary).'</td></tr>
				<tr><td colspan = "2">Medical Allowance('.$medical.'%)</td><td align="right"  valign="bottom">'.$this->Xin_model->currency_sign($medical_salary).'</td></tr>
				<tr><td colspan = "2">Transport Allowance('.$transport.'%)</td><td align="right"  valign="bottom">'.$this->Xin_model->currency_sign($transport_salary).'</td></tr>
				';
				$total = $basic_salary+$house_salary+$utilities_salary+$medical_salary+$transport_salary;
				$tblbrk.= '<tr><td colspan="2" align="center">&nbsp;</td>
				<td colspan="2" align="center" bgcolor="#A9A9A9"><strong>MONTHLY PAY</strong></td>
				</tr><tr><td colspan="2" align="center">'.ucwords($this->Xin_model->convertNumberToWord($total)).'</td>
				<td colspan="2" align="right"><strong>'.$this->Xin_model->currency_sign($total).'</strong></td>
				</tr></table>';
				$pdf->writeHTML($tblbrk, true, false, true, false, '');


				$others = '<table cellpadding="3" cellspacing="0" border="1"><tr bgcolor="#A9A9A9">
				<td colspan="2" align="center"><strong>Other Allowances</strong></td>
				<td align="center"><strong>'.$this->lang->line('xin_payslip_earning').'</strong></td>	
			</tr>';
				if(count($allowances_titles)>0)
				{
					for($i = 0; $i<count($allowances_titles);$i++)
					{
						$others.='<tr><td colspan = "2">'.$allowances_titles[$i].'</td><td align="right"  valign="bottom">'.$this->Xin_model->currency_sign($allowances_amount_in[$i]).'</td></tr>
						';
					}
				}
			} else {
				$others .= '<tr>
					...
					<td align="right"  valign="bottom">'.$this->Xin_model->currency_sign($total_count).'</td>	
				</tr>';
			}			
						//COMMISSIONS FIND
						$commission_total = 0;
						$commission_count = $this->Payroll_model->getCommission($user[0]->user_id);
						$commissions = $commission_count->num_rows();
						if($commission_count->num_rows()>0)
						{
							$commissions = $commission_count->result_array();
							foreach($commissions as $c)
							{
								$commission_total += $c['commission_amount']; 
							}
							$others .= '<tr><td colspan = "2">Commissions</td><td align="right"  valign="bottom">'.$this->Xin_model->currency_sign($commission_total).'</td></tr>';
						}
			...
			if($count_allowances > 0) {
				...
				$others .= '<tr>
					...
					<td align="right">'.$this->Xin_model->currency_sign($sl_allowances->allowance_amount).'</td>	
					</tr>';
				}
			}
			//commissions
			if($count_commissions > 0) {
				...
				$others .= '<tr>
					...
					<td align="right">'.$this->Xin_model->currency_sign($sl_commissions->commission_amount).'</td>	
					<td>&nbsp;</td>				
					</tr>';
				}
			}
			...
			if($count_other_payments > 0) {
				...
				$others .= '<tr>
					...
					<td align="right">'.$this->Xin_model->currency_sign($sl_other_payments->payments_amount).'</td>	
					<td>&nbsp;</td>				
					</tr>';
				}
			}
			...
			if($count_overtime > 0) {
				...
				$others .= '<tr>
					...
					<td align="right">'.$this->Xin_model->currency_sign($overtime_total).'</td>	
					<td>&nbsp;</td>				
					</tr>';
				}
			}
			...
			if($count_statutory_deductions > 0) {
				...
				$others .= '<tr>
					...
					<td>&nbsp;</td>
					<td align="right">'.$this->Xin_model->currency_sign($xstatutory_deduction_amount).'</td>			
					</tr>';
				}
			}
			//loan
			...
			<tr><td colspan="2" align="center"><strong>Total</strong></td>
					...
					<td colspan="2" align="right"><strong>'.$this->Xin_model->currency_sign($fsalary).'</strong></td>
					</tr></table>';
			} else {
				$total_earning = $bs + $allowances_amount + $commission_total;// + $overtime_amount + $commissions_amount + $other_payments_amount;
				$total_deduction = $loan_de_amount + $statutory_deduction_amount;
				$total_net_salary = $total_earning - $total_deduction;
				$others .= '
			<tr><td colspan="2" align="center"><strong>Total</strong></td>
					<td align="right"><strong>'.$this->Xin_model->currency_sign($total_earning).'</strong></td>
					</tr></table>
					<table cellpadding="3" cellspacing="0" border="1">
					<tr><td colspan="2" align="center">&nbsp;</td>
					<td colspan="2" align="center" bgcolor="#A9A9A9"><strong>MONTHLY PAY</strong></td>
					</tr><tr><td colspan="2" align="center">'.ucwords($this->Xin_model->convertNumberToWord($total_net_salary)).'</td>
					<td colspan="2" align="right"><strong>'.$this->Xin_model->currency_sign($total_net_salary).'</strong></td>
					</tr></table>';
			}
			if(count($allowances_titles)>0 || $commissions>0){
				$pdf->writeHTML($others, true, false, true, false, '');
			}
		// TAX Deductionssss ////
		$tax_amount = $this->Payroll_model->getTaxAmount($user[0]->user_id , $total_net_salary);
		...
		// EOBI Deductions
		$eobi = $this->Payroll_model->getEOBIamount();
		$eobi_employer_amount = $eobi[0]['eobi_employer_amount'];
		$eobi_employee_amount = $eobi[0]['eobi_amount'];
		$eobi_employee['employee_id'] = $user[0]->user_id;
		$eobi_employee['company_id'] = $user[0]->company_id;
		$eobi_employee['eobi_amount'] = ($eobi_employee_amount+$eobi_employer_amount);	
		$eobi_employee['month_year'] = $year.'-'.$m;
		$this->Payroll_model->saveEOBI($eobi_employee);
		/// table Deductions
		$provident_fund = $template[0]['provident_fund'];
		$provident_deduction = ceil(($basic_salary/100)*$provident_fund);
		$save_provident['pfa_total'] = $provident_deduction*2;
		$save_provident['company_id'] = $user[0]->company_id;
		$save_provident['employee_id'] = $user[0]->user_id;
		$save_provident['month_year'] = $year.'-'.$m;
		$this->Payroll_model->savePFA($save_provident);
		$total_salary_deduction = $provident_deduction+$tax_amount+$eobi_employee_amount;

		// Total Leaves Calculate
		$total_leaves = $this->Payroll_model->get_leaves_count($user[0]->user_id, $m);
		//Per Day Salary..
		$d=cal_days_in_month(CAL_GREGORIAN,$m,date('Y'));
		$per_day = $basic_salary/$d;
		$present_count = ($pcount+$late+$total_leaves);
		$total_days_salary = ($present_count*$per_day);
		// Total Salary Deduction
		$total_net_salary = $total_net_salary-$basic_salary+$total_days_salary;
		$total_net_salary = ceil($total_net_salary-$total_salary_deduction);
		$tbldeductions = '<table cellpadding="3" cellspacing="0" border="1"><tr bgcolor="#A9A9A9">
		<td align="center" colspan="2"><strong>Total Deductions</strong></td>
		<td align="center"><strong>Amount</strong></td>
		</tr>
		<tr><td colspan="2">Loan</td><td align="right">'.$this->Xin_model->currency_sign($loan_de_amount).'</td></tr>
		<tr><td colspan="2">EOBI</td><td align="right">'.$this->Xin_model->currency_sign($eobi_employee_amount).'</td></tr>
		<tr><td colspan="2">Provident Fund('.$provident_fund.'% on Basic)</td><td align="right">'.$this->Xin_model->currency_sign($provident_deduction).'</td></tr>
		<tr><td colspan="2">WH Tax</td><td align="right">'.$this->Xin_model->currency_sign($tax_amount).'</td></tr>
		<tr><td colspan="2" bgcolor="#A9A9A9"><strong>Net Payment</strong></td><td align="right"><strong>'.$this->Xin_model->currency_sign($total_net_salary).'</strong></td></tr>

		</table>';
		$pdf->writeHTML($tbldeductions, true, false, true, false, '');
