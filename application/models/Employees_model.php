...
class Employees_model extends CI_Model {
	...
	public function add($data, $cnic){
		$this->db->insert('xin_employees', $data);
		if ($this->db->affected_rows() > 0) {
			$cnic_data['employee_id'] = $this->db->insert_id();
			$cnic_data['cnic'] = $cnic;
			$this->db->insert('xin_cnic', $cnic_data);
			return $cnic_data['employee_id'];
		} else {
			return false;
		}
	}
   ...
   // get employee allowances
	...
	public function template_update($data)
	{
		...
		$this->db->update('xin_salary_template', $data);
		return true;
	}
	public function getTemplate()
	{
	$this->db->where('company_id',1);
	$this->db->select('basic_salary, house_rent_allowance, dearness_allowance, travelling_allowance, medical_allowance, provident_fund');
