...
$total_salary = $commissions_amount+$basic_salary+$allowance_amount;
$statutory_deductions = $this->Payroll_model->set_employee_statutory_deductions($user_id, $basic_salary,$total_salary , $absent_count);
$statutory_deductions = $total_salary-$statutory_deductions;
// 7: overtime
...
// add amount
$net_salary_default = $add_salary - $loan_de_amount - $statutory_deductions;
$sta_salary = $allowance_amount + $basic_salary;

$estatutory_deductions = $statutory_deductions;
...
<?php echo form_open('admin/payroll/add_pay_monthly/', $attributes, $hidden);?>
   <div class="row">
	...
	<div class="row">
      <div class="col-md-5">
        <div class="form-group">
          <label for="name"><?php echo "Total Commissions";?></label>
          <input type="text" name="total_commissions" class="form-control" value="<?php echo $commissions_amount;?>" readonly="readonly">
        </div>
      </div>
	  <div class="col-md-5">
        <div class="form-group">
          <label for="name"><?php echo "Total Deductions";?></label>
          <input type="text" name="total_statutory_deductions" class="form-control" value="<?php echo $statutory_deductions;?>" readonly="readonly">
        </div>
      </div>
    ...
    <div class="row">
      <div class="col-md-12">
        <div class="form-group">
          <span><strong>NOTE:</strong> <?php echo $this->lang->line('xin_payroll_total_allowance');?>,<?php echo $this->lang->line('xin_hr_commissions');?>,<?php echo $this->lang->line('xin_payroll_total_loan');?>,<?php echo $this->lang->line('xin_payroll_total_overtime');?>,<?php echo $this->lang->line('xin_employee_set_other_payment');?> are not editable.</span>
        </div>
      </div>
    </div> 
    <div class="form-actions box-footer"> <?php echo form_button(array('name' => 'hrsale_form', 'type' => 'submit', 'class' => $this->Xin_model->form_button_class(),'onclick'=>'window.location.reload(true);', 'content' => '<i class="fas fa-check-square"></i> '.$this->lang->line('xin_pay'))); ?> </div>
  <?php echo form_close(); ?>
</div>
</div>
...
// 6: statutory deductions
$statutory_deductions = $this->Payroll_model->set_employee_statutory_deductions($user_id, $fsalary);
if(!is_null($statutory_deductions)):
	...
	foreach($statutory_deductions->result() as $sl_statutory_deductions) {
		if($system[0]->statutory_fixed!='yes'):
			...
			$statutory_deductions += $st_amount;
		else:
			$statutory_deductions += $sl_statutory_deductions->deduction_amount;
		endif;
	}
endif;
