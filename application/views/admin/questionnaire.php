...
<p class="o-error"></p>
<button class="btn btn-primary" type="submit" id="submit-f" onClick="window.location.reload();">Submit</button>
</form>
   </div>
</div>
...
<tr id="row-<?php echo $q->question_id;?>">
<td class="rowId"><?php echo $q->question_id; ?></td>
<td class="rowQ"><?php echo $q->question; ?></td>
<td class="rowA"><?php echo $q->answers;?></td>
<td><span data-toggle="tooltip" data-placement="top" data-state="primary" title="Edit"><a href="" type="button" class="btn icon-btn btn-sm btn-outline-secondary waves-effect waves-light edit-data" data-toggle="modal" onclick="edit(<?php echo $q->question_id;?>)"><span class="fas fa-pencil-alt"></span></a></span><span data-toggle="tooltip" data-placement="top" data-state="danger" title="<?php $this->lang->line('xin_delete')?> "><button type="button" class="btn icon-btn btn-sm btn-outline-danger waves-effect waves-light delete" data-toggle="modal" onclick="deleteq(<?php echo $q->question_id ?>)" ><span class="fas fa-trash-restore"></span></button></span>
</tr>
<?php } ?>
</table>
</div>
...
<div class="modal" id="delete-M">
<div class="modal-dialog">
<div class="modal-content">
<div class="modal-header">
<h5>Do you really want to delete Question?</h5>
<button type="button" class="close" data-dismiss="modal" aria-label="Close">
<span aria-hidden="true">&times;</span>
        </button>
        </div>
...
$("body").on("submit", "#question-f", function(e){
    ...
    $.ajax({
        ...
        {if(response['status'] == 0){
          ...
          {
            $('.q-error').html(response['question']).addClass('invalid-feedback d-block');
            $('#question').addClass('is-invalid');

          }
          if(response['answer'] != '')
          {
            $('.a-error').html(response['answer']).addClass('invalid-feedback d-block');
            $('#answer').addClass('is-invalid');

          }
          if(response['options'] != '')
          {
            $('.o-error').html(response['options']).addClass('invalid-feedback d-block');
...
function edit(id){
  $("#edit-M").modal('show');
  $.ajax({
    url : "<?php echo base_url()?>hrms-api/index.php/Questionnaire/editQuestion/"+id,
    type : "POST",
    data :{},
    dataType: 'json',
    success : function(response)
    {
      $("#response").html(response['modal_body'])
    }
  });
}
function deleteq(id)
{
  $("#delete-M").modal('show');
  $.ajax({
    url : "<?php echo base_url()?>hrms-api/index.php/Questionnaire/getRow/"+id,
    type : "POST",
    data : {},
    dataType:'json',
    success : function(response)
    {
      $(".delete_response").html(response['body']);
    }
  });
}
$("body").on("submit", "#delete-f", function(e)
{
  e.preventDefault();
  $.ajax({
    url : "<?php echo base_url()?>hrms-api/index.php/Questionnaire/deleteQ",
    type : "POST",
    data : $(this).serializeArray(),
    dataType:'json',
    success : function(response)
...
$("body").on("submit","#edit-form",function(e){
  e.preventDefault();
  $.ajax({
    url : "<?php echo base_url()?>hrms-api/index.php/questionnaire/saveEditQ",
    type :"POST",
    data : $(this).serializeArray(),
    dataType : 'json',
    success : function(response)
    {
      if(response['status'] == 0){
          if(response['question'] != '')
          {
            $('.ques-error').html(response['question']).addClass('invalid-feedback d-block');
          }
          if(response['answer'] != '')
          {
            $('.ans-error').html(response['answer']).addClass('invalid-feedback d-block');
          }
          if(response['options'] != '')
          {
            $('.opt-error').html(response['options']).addClass('invalid-feedback d-block');
          }
        }
        else{
          $("#edit-M").modal('hide');
          $("#row-"+response['id']+" .rowId").html(response['id']);
          $("#row-"+response['id']+" .rowQ").html(response['question']);
          $("#row-"+response['id']+" .rowA").html(response['answers']);
        }
    }
  });
})
