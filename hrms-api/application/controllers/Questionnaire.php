...
class Questionnaire extends CI_Controller {
    ...
    public function editQuestion($id)
    {
        ...
        $response['modal_body'] = $this->load->view('dialog_edit', $question,true);
        echo json_encode($response);
        return json_encode($response);
    }
    public function saveEditQ()
    {
        ...
        else
        {
            ...
            $row['data'] = $this->Questionnaire_model->editQues($data, $id);
            $response['status'] = 1;
            $response['data'] = $this->load->view('check', $row, true);
            $response['id'] = $id;
        }
    ...
    public function deleteQ()
    {
        ...
        $this->Questionnaire_model->deleteQuestion($id);
        $response['id'] =$id;
        $response['status'] = 1;
        echo json_encode($response);
        return json_encode($response);
    ...
    {
        $result['data'] = $this->Questionnaire_model->fetchQuestion($id);
        $response['body'] = $this->load->view('deleteRow',$result,true);
        $response['status'] = 1;
        echo json_encode($response);
        return json_encode($response);
    }
