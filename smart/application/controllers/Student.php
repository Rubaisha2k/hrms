...
class Student extends Admin_Controller
{
    ...
    public function create()
    {
        ...
        $this->form_validation->set_rules('guardian_nic', $this->lang->line('guardian_nic'), 'trim|required|xss_clean');
        ...
        if ($this->form_validation->run() == false) {
            ...
            $data_insert = array(
                ...
                'adhar_no' => $this->input->post('guardian_nic'),
            ...
            $tree = $this->student_model->getByNic($this->input->post('guardian_nic'));
            if ($tree == 1) {
                $this->session->set_flashdata('msg', '<div class="alert alert-danger text-center">' . "Guardian NIC Already Exists" . '</div>');
                ...
                if ($insert) {
                    ...
                    if ($sibling_id > 0) {
                        ...
                        $data_parent_login = array(
                            //yahha parent username hai
                            'username' => $this->input->post('guardian_nic'),
                    ...
                    $this->session->set_flashdata('msg','<script type="text/javascript"> '. " $(document).ready(function () { $('#modal_popup').modal({backdrop: 'static', keyboard: true, show: true})   });". ' </script>');
    ...
    public
    function handle_uploadcreate_doc()
    {
    ...
    public
    function handle_guardian_upload()
    {
...
public
    function import()
    {
        ...
        $userdata = $this->customlib->getUserData();
        $category = $this->category_model->get();
        $fields = array('admission_no', 'roll_no', 'firstname', 'lastname', 'gender', 'dob',
            'category_id', 'religion', 'cast', 'mobileno', 'email', 'admission_date', 'blood_group',
            'school_house_id', 'height', 'weight', 'measurement_date', 'father_name', 'father_phone',
            'father_occupation', 'mother_name', 'mother_phone',
            'mother_occupation', 'guardian_is', 'guardian_name', 'guardian_relation', 'guardian_email',
            'guardian_phone', 'guardian_occupation', 'guardian_address', 'current_address', 'permanent_address',
            'bank_account_no', 'bank_name', 'ifsc_code', 'adhar_no', 'samagra_id', 'rte', 'previous_school', 'note');
        $data["fields"] = $fields;
        $data['categorylist'] = $category;
        $this->form_validation->set_rules('class_id', $this->lang->line('class'), 'trim|required|xss_clean');
        $this->form_validation->set_rules('section_id', $this->lang->line('section'), 'trim|required|xss_clean');
...
//                    echo "<pre>";print_r($result);echo "<pre>";die;
                    if (!empty($result)) {
                        ...
                        for ($i = 1; $i <= count($result); $i++) {
                            ...
                            $mobile_no = "0".$student_data[$i]["mobileno"];
                            $email = $student_data[$i]["email"];
                            $guardian_phone = $student_data[$i]["guardian_phone"];
                            $guardian_email = $student_data[$i]["guardian_email"];
                            $adhar_no = $student_data[$i]["adhar_no"];
                            $data_setting = array();
                            $data_setting['id'] = $this->sch_setting_detail->id;
                            $data_setting['adm_auto_insert'] = $this->sch_setting_detail->adm_auto_insert;
                            $data_setting['adm_update_status'] = $this->sch_setting_detail->adm_update_status;

                            //echo "<pre>";print_r($student_data[$i]["adhar_no"].$i);echo "<pre>";die;
                            ...
                            if (!empty($insert_id)) {
                                ...
                                $temp = $insert_id;
                                $data_parent_login = array(
                                'username' => number_format($student_data[$i]["adhar_no"],0,'',''),
 'password' => 'ngss123',
                                    'user_id' => $insert_id,
                                    'role' => 'parent',
                                    'childs' => $temp,
