...
class Onlinestudent extends Admin_Controller
{
    ...
    public function edit($id)
    {
       ...
       $data['admission_no_au'] = $admission_no;
        ...
        if ($this->form_validation->run() == false) {
            ...
            $tree = $this->onlinestudent_model->getByNic($this->input->post('guardian_nic'));
            if ($tree==1) {
                $this->form_validation->set_rules('guardian_nic', 'Guardian NIC already exists', 'trim|required|xss_clean');
                $this->session->set_flashdata('msg', '<div class="alert alert-success text-left">' . 'Guardian NIC Already Exists ' . '</div>');
      ...
      $this->session->set_flashdata('msg', '<script type="text/javascript"> ' . " $(document).ready(function () { $('#modal_popup').modal({backdrop: 'static', keyboard: true, show: true})   });" . ' </script>');
                    redirect('admin/onlinestudent/edit/'.$student_id);
