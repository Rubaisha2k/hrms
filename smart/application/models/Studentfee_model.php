...
class Studentfee_model extends CI_Model {
    ...
    public function getStudent_Bulkfee($adm_no)
    {
        $this->db->select('admission_no, student_session_id, fees_amount, class_id, student_id, section_id, class, section');
        $this->db->join('student_session', 'student_session.id= generate_voucher.student_session_id');
        ...
        $this->db->from('students');
         $this->db->where('id', $result->student_id);
        $result2 = $this->db->get();
        //print_r($result2->row()); 
        return ($result);
        return($result2->row());
