    ...
    public function fee_deposit($data, $send_to, $student_fees_discount_id = null)
    {
        ...
        if ($q->num_rows() > 0) {
            ...
            if ($student_fees_discount_id != null) {
                ...
                $data_new_discount = array(
                    'student_session_id' => $wasay['student_session_id'],
                    'fees_discount_id' => $wasay['fees_discount_id'],
                    'status' => 'assigned',
                    'payment_id' => '',
                    'description' => 'Create When Assigned',
                    'is_active' => 'yes'
                );

                $this->db->insert('student_fees_discounts', $data_new_discount);
...
//and student_session.session_id='".$this->current_session."'
    ...
    public function getPreviousBalance($class_id, $section_id)
    {
        // if($month_year==0)
        // {
        //     $month_year = 12;
        // }
        $this->db->select('student_session_id, amount, firstname, lastname, admission_no, roll_no, class, section');
    ...
    public function addmonth_year($student_data, $due_date)
    {
        $this->db->trans_start();
        $this->db->trans_strict(false);
        $fee_group_exists = $this->feegroup_model->checkGroupExistsByName($this->balance_group);
